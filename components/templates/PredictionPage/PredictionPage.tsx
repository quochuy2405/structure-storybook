...
import { Button, CheckBox, Switch, Title } from '@/components/atoms'
import {
  ButtonPredict,
  CheckboxGroupForm,
  ModelReview,
...
const PredictionPage: React.FC<IPredictionPageProps> = ({
  ...
  return (
    <div className={Styles.TradingView}>
      {query?.coin && introPredict[query?.coin as string]}
            ...
            {!switchForm ? (
              <div className={Styles.InTroPredict}>
                <div className={Styles.IconCoin}>
                  {query?.coin && coinIcons[query?.coin as string]}
                </div>
                <Title size={1.7}>
                  Trading {coinTexts[query?.coin as string] || ''} with the world{"'"}s
